...
LONG CSerial::Setup (EBaudrate eBaudrate, EDataBits eDataBits, EParity eParity, EStopBits eStopBits)
{
	...
	///////////////////////////////MODIFIED BY MUSTAFA BEDRI SEN////////////////////////////////////
	
	dcb.fRtsControl = RTS_CONTROL_ENABLE;

	////////////////////////END OF MODIFIED BY MUSTAFA BEDRI SEN////////////////////////////////////
...
LONG CSerial::Write (const void* pData, size_t iLen, DWORD* pdwWritten, LPOVERLAPPED lpOverlapped, DWORD dwTimeout)
{
	...
	*pdwWritten = 0;
	...
	//failure = SetCommMask(m_hFile, EV_TXEMPTY);

	failure = EscapeCommFunction( m_hFile, CLRRTS ); //clear the rts line so that computer talks on rs485 bus 
...
/*	if (!::WriteFile(m_hFile,pData,iLen,pdwWritten,lpOverlapped))
	...
	*/
	...
	failure = WriteFile(m_hFile,pData,iLen,pdwWritten,lpOverlapped);
...
//	DWORD dwEventMask;
	...
	//if(WaitCommEvent(m_hFile, &dwEventMask, NULL))
	

	failure = EscapeCommFunction( m_hFile, SETRTS ); //set the rts line so that computer listens on rs485 bus 
